...
<section class="section-padding">
  <div class="container container-wide">
    <h1 style="margin-bottom:var(--space-xl);"><?php wpm_is_rtl() ? _e('الدفع والتسجيل الآمن', 'edtech') : _e('Secure Checkout', 'edtech'); ?></h1>
    ...
    <div class="layout-content-aside-wide">
      ...
      <div>
        ...
        <?php if ( ! $is_logged_in ) : ?>
          ...
          <div class="card reveal" style="margin-bottom:var(--space-lg);border-radius:var(--radius-lg);padding:var(--space-xl);">
            <div style="margin-bottom:var(--space-md);">
              <span class="badge badge-bestseller" style="margin-bottom:var(--space-xs);"><?php wpm_is_rtl() ? _e('خطوة 1 من 2', 'edtech') : _e('Step 1 of 2', 'edtech'); ?></span>
              <h3 style="margin-bottom:var(--space-xs);"><?php wpm_is_rtl() ? _e('تسجيل الدخول أو إنشاء حساب للمتابعة', 'edtech') : _e('Sign In or Register to Continue', 'edtech'); ?></h3>
              <p style="font-size:13px;color:var(--color-text-muted);margin:0;"><?php wpm_is_rtl() ? _e('سجّل دخولك لحفظ بيانات الدورة في حسابك ومتابعتها من أي جهاز', 'edtech') : _e('Sign in to attach this course to your account and track progress anywhere', 'edtech'); ?></p>
            </div>
            ...
            <div class="tabs" style="margin-bottom:var(--space-lg);grid-template-columns:1fr 1fr;display:grid;">
              <button class="tab-trigger <?php echo $active_tab === 'login' ? 'active' : ''; ?>" onclick="switchCheckoutAuthTab('login')" id="co-tab-login">
                <?php wpm_is_rtl() ? _e('تسجيل الدخول الحساب', 'edtech') : _e('Sign In', 'edtech'); ?>
              </button>
              <button class="tab-trigger <?php echo $active_tab === 'register' ? 'active' : ''; ?>" onclick="switchCheckoutAuthTab('register')" id="co-tab-register">
                <?php wpm_is_rtl() ? _e('إنشاء حساب جديد', 'edtech') : _e('Register Account', 'edtech'); ?>
              </button>
            </div>
            ...
            <div id="co-panel-login" style="display:<?php echo $active_tab === 'login' ? 'block' : 'none'; ?>;">
              <form method="post" action="<?php echo esc_url( $checkout_self ); ?>">
                ...
                <div style="margin-bottom:var(--space-sm);">
                  <label for="co-log" style="display:block;font-size:12px;font-weight:600;margin-bottom:4px;"><?php wpm_is_rtl() ? _e('اسم المستخدم أو البريد', 'edtech') : _e('Username or Email', 'edtech'); ?></label>
                  <input type="text" name="log" id="co-log" class="input-field" placeholder="user@example.com" required style="width:100%;">
                </div>

                <div style="margin-bottom:var(--space-sm);">
                  <label for="co-pwd" style="display:block;font-size:12px;font-weight:600;margin-bottom:4px;"><?php wpm_is_rtl() ? _e('كلمة المرور', 'edtech') : _e('Password', 'edtech'); ?></label>
                  <input type="password" name="pwd" id="co-pwd" class="input-field" placeholder="••••••••" required style="width:100%;">
                </div>

                <button type="submit" class="btn btn-secondary" style="width:100%;margin-top:var(--space-xs);"><?php wpm_is_rtl() ? _e('تسجيل الدخول والمتابعة ←', 'edtech') : _e('Sign In & Continue ←', 'edtech'); ?></button>
              </form>
            </div>
            ...
            <div id="co-panel-register" style="display:<?php echo $active_tab === 'register' ? 'block' : 'none'; ?>;">
              <form method="post" action="<?php echo esc_url( $checkout_self ); ?>">
                ...
                <div style="margin-bottom:var(--space-sm);">
                  <label for="co-reg-user" style="display:block;font-size:12px;font-weight:600;margin-bottom:4px;"><?php wpm_is_rtl() ? _e('اسم المستخدم', 'edtech') : _e('Username', 'edtech'); ?></label>
                  <input type="text" name="username" id="co-reg-user" class="input-field" placeholder="student_name" required style="width:100%;">
                </div>

                <div style="margin-bottom:var(--space-sm);">
                  <label for="co-reg-email" style="display:block;font-size:12px;font-weight:600;margin-bottom:4px;"><?php wpm_is_rtl() ? _e('البريد الإلكتروني', 'edtech') : _e('Email Address', 'edtech'); ?></label>
                  <input type="email" name="email" id="co-reg-email" class="input-field" placeholder="student@example.com" required style="width:100%;">
                </div>

                <div style="margin-bottom:var(--space-sm);">
                  <label for="co-reg-pwd" style="display:block;font-size:12px;font-weight:600;margin-bottom:4px;"><?php wpm_is_rtl() ? _e('كلمة المرور', 'edtech') : _e('Create Password', 'edtech'); ?></label>
                  <input type="password" name="password" id="co-reg-pwd" class="input-field" placeholder="••••••••" required style="width:100%;">
                </div>

                <button type="submit" class="btn btn-secondary" style="width:100%;margin-top:var(--space-xs);"><?php wpm_is_rtl() ? _e('إنشاء الحساب والمتابعة ←', 'edtech') : _e('Create Account & Continue ←', 'edtech'); ?></button>
              </form>
            </div>
        ...
        <?php else : ?>
          ...
          <div class="card reveal" style="margin-bottom:var(--space-lg);background:linear-gradient(145deg, var(--color-bg-subtle) 0%, var(--color-bg-card) 100%);padding:var(--space-md) var(--space-lg);border-inline-start:4px solid var(--color-success);border-radius:var(--radius-md);">
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:var(--space-xs);">
              <div>
                <span style="font-size:12px;color:var(--color-success);font-weight:700;"><?php wpm_is_rtl() ? _e('✓ مسجّل الدخول حالياً', 'edtech') : _e('✓ Currently Signed In', 'edtech'); ?></span>
              ...
              <a href="<?php echo esc_url( wp_logout_url( $checkout_self ) ); ?>" style="font-size:12px;color:var(--color-text-muted);text-decoration:underline;"><?php wpm_is_rtl() ? _e('تبديل الحساب', 'edtech') : _e('Switch Account', 'edtech'); ?></a>
            </div>
          </div>
        <?php endif; ?>
        ...
        <div class="card reveal">
          <h3 style="margin-bottom:var(--space-lg);"><?php wpm_is_rtl() ? _e('إتمام التسجيل', 'edtech') : _e('Complete Enrollment', 'edtech'); ?></h3>
          <?php if ( $course_id && $course_meta ) : ?>
            <?php if ( $is_logged_in ) : ?>
              <p style="font-size:14px;color:var(--color-text-muted);margin-bottom:var(--space-md);">
                <?php wpm_is_rtl() ? _e('اضغط الزر أدناه للتسجيل في الدورة. سيتم إضافتها فوراً إلى لوحة تحكم الطالب.', 'edtech') : _e('Click the button below to enroll. The course will be added to your student dashboard instantly.', 'edtech'); ?>
              </p>
              <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                ...
                <button type="submit" class="btn btn-primary btn-lg" style="width:100%;">
                  <?php wpm_is_rtl() ? printf( __( 'إتمام التسجيل — $%s', 'edtech' ), esc_html( $course_meta['price'] ) ) : printf( __( 'Complete Enrollment — $%s', 'edtech' ), esc_html( $course_meta['price'] ) ); ?>
                </button>
              </form>
            <?php else : ?>
              <p style="font-size:14px;color:var(--color-text-muted);"><?php wpm_is_rtl() ? _e('سجّل الدخول أو أنشئ حساباً بالأعلى لإتمام التسجيل.', 'edtech') : _e('Sign in or register above to complete enrollment.', 'edtech'); ?></p>
            <?php endif; ?>
          <?php else : ?>
            <p style="font-size:14px;color:var(--color-text-muted);"><?php wpm_is_rtl() ? _e('لم يتم اختيار دورة. تصفح الكتالوج لاختيار دورة.', 'edtech') : _e('No course selected. Browse the catalog to choose a course.', 'edtech'); ?></p>
      ...
      <aside>
        <div class="card">
          <h3 style="margin-bottom:var(--space-md);"><?php wpm_is_rtl() ? _e('ملخص الطلب', 'edtech') : _e('Order Summary', 'edtech'); ?></h3>
          <?php if ( $course_id && $course_meta ) : ?>
            ...
            <div style="display:flex;justify-content:space-between;font-weight:700;font-size:1.1rem;margin-top:var(--space-sm);">
              <span><?php wpm_is_rtl() ? _e('المجموع الإجمالي', 'edtech') : _e('Total', 'edtech'); ?></span>
              <span style="color:var(--color-primary);">$<?php echo esc_html( $course_meta['price'] ); ?></span>
            </div>
          <?php else : ?>
            <p style="font-size:14px;color:var(--color-text-muted);"><?php wpm_is_rtl() ? _e('لا توجد دورة محددة.', 'edtech') : _e('No course selected.', 'edtech'); ?></p>
          <?php endif; ?>
        </div>
      </aside>
