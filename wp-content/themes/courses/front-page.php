...
$hero_title_raw = edtech_get_site_setting( 'hero_title', wpm_is_rtl() ? 'أتقن المهارات الرقمية||الأكثر طلباً|| وابنِ مشاريع حقيقية' : 'Master High-Impact||Digital Skills|| & Build Real Products' );
$hero_title_parts = array_map( 'trim', explode( '||', $hero_title_raw ) );
$hero_eyebrow   = edtech_get_site_setting( 'hero_eyebrow', wpm_is_rtl() ? 'تقييم 4.9' : '4.9 Rating' );
$hero_subtitle  = edtech_get_site_setting( 'hero_subtitle', wpm_is_rtl() ? '+45,000 طالب نشط' : '45,000+ Active Students' );
$hero_desc      = edtech_get_site_setting( 'hero_description', wpm_is_rtl() ? 'توقف عن مشاهدة الشروحات السطحية. تعلّم من خبراء الصناعة المعتمدين، وابنِ مشاريع قوية لمحفظتك.' : 'Stop watching passive tutorials. Learn from verified industry practitioners, build portfolio-grade projects, and launch your digital career.' );
$hero_cta       = edtech_get_site_setting( 'hero_cta_label', wpm_is_rtl() ? 'اشترك الآن — 49$' : 'Enroll Now — $49' );
$hero_cta2      = edtech_get_site_setting( 'hero_cta2_label', wpm_is_rtl() ? 'شاهد الماستر كلاس المجاني' : 'Watch Free Masterclass' );
      ...
      <div class="reveal">
        ...
        <div class="input-search-wrapper" style="max-width:520px;margin-bottom:var(--space-lg);">
          ...
          <input
            ...
            placeholder="<?php wpm_is_rtl() ? _e('ابحث عن دورات أو مهارات أو مدربين…', 'edtech') : _e('Search courses, skills, or instructors…', 'edtech'); ?>"
        ...
        <div style="display:flex;gap:var(--space-xs);flex-wrap:wrap;margin-bottom:var(--space-xl);">
          ...
          if ( ! is_wp_error( $chip_terms ) && $chip_terms ) :
            ...
            <a href="<?php echo esc_url( $catalog_link ); ?>" class="chip"><?php wpm_is_rtl() ? _e('تصفح الدورات', 'edtech') : _e('Browse Courses', 'edtech'); ?></a>
          <?php endif; ?>
        </div>
        ...
        <div style="display:flex;gap:var(--space-lg);margin-top:var(--space-xl);flex-wrap:wrap;">
          <div style="display:flex;align-items:center;gap:6px;font-size:13px;color:var(--color-text-muted);">
            ...
            <?php wpm_is_rtl() ? _e('ضمان استرداد الأموال خلال 30 يوماً', 'edtech') : _e('30-Day Money-Back Guarantee', 'edtech'); ?>
          </div>
          <div style="display:flex;align-items:center;gap:6px;font-size:13px;color:var(--color-text-muted);">
            ...
            <?php wpm_is_rtl() ? _e('تشفير SSL آمن 256-bit', 'edtech') : _e('256-bit SSL Secure Checkout', 'edtech'); ?>
          </div>
        </div>
      </div>
      ...
      <div class="reveal" style="--reveal-delay:100ms;">
        <div class="card" style="padding:0;overflow:hidden;border-radius:var(--radius-lg);box-shadow:var(--elevation-hover);">
          <div style="display:flex;border-bottom:1px solid var(--color-border-subtle);background:var(--color-bg-subtle);">
            <button class="tab-trigger active" data-panel="sandbox-dev" style="flex:1;border-radius:0;height:48px;font-size:13px;"><?php wpm_is_rtl() ? _e('تطبيق متكامل', 'edtech') : _e('Full-Stack App', 'edtech'); ?></button>
            <button class="tab-trigger" data-panel="sandbox-des" style="flex:1;border-radius:0;height:48px;font-size:13px;"><?php wpm_is_rtl() ? _e('نظام Figma', 'edtech') : _e('Figma UI System', 'edtech'); ?></button>
          </div>
          <div data-tabs-container>
            <div id="sandbox-dev" class="tab-panel active" style="padding:0;position:relative;aspect-ratio:4/3;overflow:hidden;">
              ...
              <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,0.7),transparent);display:flex;flex-direction:column;justify-content:flex-end;padding:var(--space-md);">
                <span class="badge badge-free" style="align-self:flex-start;margin-bottom:8px;"><?php wpm_is_rtl() ? _e('مشروع طالب حقيقي', 'edtech') : _e('Student Project Output', 'edtech'); ?></span>
                <p style="color:white;font-size:14px;font-weight:600;margin:0;"><?php wpm_is_rtl() ? _e('تطبيق متجر إلكتروني كامل بـ React وNode.js', 'edtech') : _e('Full-Stack E-Commerce App built with React & Node.js', 'edtech'); ?></p>
              </div>
            </div>
            <div id="sandbox-des" class="tab-panel" style="padding:0;position:relative;aspect-ratio:4/3;overflow:hidden;">
              ...
              <div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,0.7),transparent);display:flex;flex-direction:column;justify-content:flex-end;padding:var(--space-md);">
                <span class="badge badge-free" style="align-self:flex-start;margin-bottom:8px;"><?php wpm_is_rtl() ? _e('مشروع طالب حقيقي', 'edtech') : _e('Student Project Output', 'edtech'); ?></span>
                <p style="color:white;font-size:14px;font-weight:600;margin:0;"><?php wpm_is_rtl() ? _e('نظام تصميم Figma متكامل لمنتج SaaS', 'edtech') : _e('Complete Figma Design System for SaaS Product', 'edtech'); ?></p>
              </div>
            </div>
          </div>
          <div style="padding:var(--space-md);background:var(--color-bg-subtle);display:flex;justify-content:space-between;align-items:center;">
            <div>
              <p style="font-size:13px;color:var(--color-text-muted);margin:0;"><?php wpm_is_rtl() ? _e('من كتالوج الدورات', 'edtech') : _e('From the catalog:', 'edtech'); ?>
                <a href="<?php echo esc_url( $catalog_link ); ?>" style="color:var(--color-primary);font-weight:600;"><?php wpm_is_rtl() ? _e('استكشف الدورات', 'edtech') : _e('Explore Courses', 'edtech'); ?></a>
              </p>
            </div>
            <a href="<?php echo esc_url( edtech_page_url( 'free-masterclass' ) ); ?>" class="btn btn-primary" style="height:36px;font-size:13px;"><?php wpm_is_rtl() ? _e('جرّب مجاناً ←', 'edtech') : _e('Try Free Sample →', 'edtech'); ?></a>
          </div>
        </div>
      </div>
...
<section class="section-padding" style="background:var(--color-bg-subtle);">
  <div class="container">
    <div style="text-align:center;max-width:660px;margin-inline:auto;margin-bottom:var(--space-xl);" class="reveal">
      <h2><?php wpm_is_rtl() ? _e('تجربة درس عملي واقعي — قبل التسجيل', 'edtech') : _e('Experience a Real Lesson — Before You Enroll', 'edtech'); ?></h2>
      <p style="color:var(--color-text-muted);margin-top:var(--space-sm);"><?php wpm_is_rtl() ? _e('شاهد درساً كاملاً بجودة عالية HD مع ترجمة والتحكم في السرعة وملفات قابلة للتنزيل. بدون تسجيل.', 'edtech') : _e('Watch a full HD sample lesson with Arabic & English captions, speed control, and downloadable resources. No sign-up required.', 'edtech'); ?></p>
    </div>
...
<section class="section-padding">
  <div class="container">
    <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:var(--space-xl);flex-wrap:wrap;gap:var(--space-md);">
      <div class="reveal">
        <h2><?php wpm_is_rtl() ? _e('تعلّم من خبراء الصناعة المعتمدين', 'edtech') : _e('Learn From Verified Industry Experts', 'edtech'); ?></h2>
        <p style="color:var(--color-text-muted);margin-top:var(--space-xs);"><?php wpm_is_rtl() ? _e('اضغط على الميكروفون للاستماع لمقدمة صوتية قصيرة من المدرب.', 'edtech') : _e('Tap the microphone to hear a 30-second voice greeting from each instructor.', 'edtech'); ?></p>
      </div>
      <a href="<?php echo esc_url( get_post_type_archive_link( 'instructor' ) ); ?>" class="btn btn-secondary reveal"><?php wpm_is_rtl() ? _e('عرض جميع المدربين', 'edtech') : _e('View All Instructors', 'edtech'); ?></a>
    </div>
    ...
    <div class="grid grid-4">
      ...
      if ( $instructors_query->have_posts() ) :
        ...
        $fallback = array(
          array( 'name' => wpm_is_rtl() ? 'م. طارق منصور' : 'Eng. Tariq Mansour', 'title' => wpm_is_rtl() ? 'مهندس Full-Stack أول' : 'Senior Full-Stack Architect', 'img' => 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&auto=format&fit=crop&q=80', 'stats' => wpm_is_rtl() ? '★ 4.9 · 1,240 طالب' : '★ 4.9 · 1,240 Students' ),
          array( 'name' => wpm_is_rtl() ? 'سارة الراشد' : 'Sarah Al-Rashid', 'title' => wpm_is_rtl() ? 'مصممة UI/UX رائدة' : 'Lead UI/UX Designer', 'img' => 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=200&auto=format&fit=crop&q=80', 'stats' => wpm_is_rtl() ? '★ 4.8 · 890 طالب' : '★ 4.8 · 890 Students' ),
          array( 'name' => wpm_is_rtl() ? 'د. عمر فاروق' : 'Dr. Omar Farooq', 'title' => wpm_is_rtl() ? 'خبير علم البيانات' : 'Data Science Specialist', 'img' => 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=200&auto=format&fit=crop&q=80', 'stats' => wpm_is_rtl() ? '★ 4.9 · 2,100 طالب' : '★ 4.9 · 2,100 Students' ),
          array( 'name' => wpm_is_rtl() ? 'ليلى حسن' : 'Layla Hassan', 'title' => wpm_is_rtl() ? 'استراتيجية النمو الرقمي' : 'Digital Growth Strategist', 'img' => 'https://images.unsplash.com/photo-1580489944761-15a19d654956?w=200&auto=format&fit=crop&q=80', 'stats' => wpm_is_rtl() ? '★ 4.7 · 680 طالب' : '★ 4.7 · 680 Students' ),
        );
  ...
  <div class="container">
    <div style="max-width:860px;margin-inline:auto;">
      <div class="grid grid-2" style="align-items:center;gap:var(--space-xl);">
        <div class="reveal">
          <h2 style="margin-bottom:var(--space-md);"><?php wpm_is_rtl() ? _e('نتائج حقيقية من طلاب حقيقيين', 'edtech') : _e('Real Results From Real Students', 'edtech'); ?></h2>
          <div style="display:flex;align-items:center;gap:var(--space-md);margin-bottom:var(--space-lg);">
            ...
            <div>
              ...
              <p style="color:var(--color-text-muted);font-size:14px;margin-top:4px;"><?php wpm_is_rtl() ? _e('متوسط التقييم من أكثر من 4,400 مراجعة موثّقة', 'edtech') : _e('Average rating from 4,400+ verified reviews', 'edtech'); ?></p>
            </div>
          </div>
        ...
        <div class="reveal">
          <div class="grid grid-2 stats-grid" style="gap:var(--space-md);">
            <div class="stat-card text-center"><div class="stat-value" data-target="<?php echo esc_attr( (int) $stat_students ); ?>">0</div><div class="stat-label"><?php wpm_is_rtl() ? _e('طالب نشط', 'edtech') : _e('Active Students', 'edtech'); ?></div></div>
            <div class="stat-card text-center"><div class="stat-value" data-target="<?php echo esc_attr( $stat_courses ); ?>">0</div><div class="stat-label"><?php wpm_is_rtl() ? _e('دورة متميزة', 'edtech') : _e('Premium Courses', 'edtech'); ?></div></div>
            <div class="stat-card text-center"><div class="stat-value" data-target="<?php echo esc_attr( $stat_instructors ); ?>">0</div><div class="stat-label"><?php wpm_is_rtl() ? _e('مدرب خبير', 'edtech') : _e('Expert Instructors', 'edtech'); ?></div></div>
            <div class="stat-card text-center"><div class="stat-value"><?php echo esc_html( $stat_completion ); ?></div><div class="stat-label"><?php wpm_is_rtl() ? _e('معدل الإكمال', 'edtech') : _e('Completion Rate', 'edtech'); ?></div></div>
          </div>
        </div>
      </div>
    </div>
...
<section class="section-padding">
  <div class="container">
    <div style="text-align:center;max-width:660px;margin-inline:auto;margin-bottom:var(--space-xl);" class="reveal">
      <h2><?php wpm_is_rtl() ? _e('مسارات تعلم منظمة لتأهيلك وظيفياً', 'edtech') : _e('Structured Learning Paths to Career-Ready Skills', 'edtech'); ?></h2>
      <p style="color:var(--color-text-muted);margin-top:var(--space-xs);"><?php wpm_is_rtl() ? _e('اتبع مسارات مهنية متعددة الدورات مصممة للوصول بك للهدف بشكل أسرع.', 'edtech') : _e('Follow curated multi-course career tracks and arrive at your destination faster.', 'edtech'); ?></p>
    </div>
    ...
    <div style="max-width:720px;margin-inline:auto;" class="reveal">
      <div class="skill-tree">
        ...
        if ( $paths_query->have_posts() ) :
          ...
          while ( $paths_query->have_posts() ) : $paths_query->the_post();
            ...
            <div class="skill-node">
              <div class="skill-node-card" style="display:flex;justify-content:space-between;align-items:center;">
                <div>
                  <span class="badge <?php echo esc_attr( $badge_class ); ?>" style="margin-bottom:4px;"><?php wpm_is_rtl() ? printf( 'المرحلة %d', $phase ) : printf( 'Phase %d', $phase ); ?></span>
                  <h4 style="margin:0;"><?php the_title(); ?></h4>
                  <p style="font-size:13px;color:var(--color-text-muted);margin:4px 0 0;"><?php echo esc_html( sprintf( '%s %s · %s %s', $weeks, ( wpm_is_rtl() ? 'أسابيع' : 'Weeks' ), $pcourses, ( wpm_is_rtl() ? 'دورات' : 'Courses' ) ) ); ?></p>
                </div>
          ...
          <div class="skill-node">
            <div class="skill-node-card" style="display:flex;justify-content:space-between;align-items:center;">
              <div>
                <span class="badge badge-new" style="margin-bottom:4px;"><?php wpm_is_rtl() ? _e('المرحلة 1', 'edtech') : _e('Phase 1', 'edtech'); ?></span>
                <h4 style="margin:0;"><?php wpm_is_rtl() ? _e('أساسيات الواجهة الأمامية (HTML, CSS, React)', 'edtech') : _e('Frontend Foundations (HTML, CSS, React)', 'edtech'); ?></h4>
                <p style="font-size:13px;color:var(--color-text-muted);margin:4px 0 0;"><?php wpm_is_rtl() ? _e('4 أسابيع · 3 دورات', 'edtech') : _e('4 Weeks · 3 Courses', 'edtech'); ?></p>
              </div>
            </div>
          </div>
        <?php endif; ?>
      ...
      <div style="text-align:center;margin-top:var(--space-xl);">
        <a href="<?php echo esc_url( edtech_page_url( 'learning-paths' ) ); ?>" class="btn btn-primary btn-lg"><?php wpm_is_rtl() ? _e('استكشف جميع مسارات التعلم', 'edtech') : _e('Explore All Learning Paths', 'edtech'); ?></a>
      </div>
    </div>
  </div>
</section>
...
<section class="section-padding" style="background:linear-gradient(135deg,hsl(222,85%,56%) 0%,hsl(262,80%,50%) 100%);">
  <div class="container" style="text-align:center;">
    <div class="reveal">
      <h2 style="color:white;font-size:var(--font-size-h1);margin-bottom:var(--space-md);"><?php echo esc_html( edtech_get_site_setting( 'enroll_banner_title', wpm_is_rtl() ? 'هل أنت جاهز لبناء مستقبلك؟' : 'Ready to Build Your Future?' ) ); ?></h2>
      <p style="color:rgba(255,255,255,0.85);font-size:var(--font-size-body-lg);margin-bottom:var(--space-xl);max-width:560px;margin-inline:auto;">
        <?php echo esc_html( edtech_get_site_setting( 'enroll_banner_text', wpm_is_rtl() ? 'انضم إلى 45,000+ طالب يتعلمون بالفعل. ابدأ بالماستر كلاس المجاني — لا حاجة لبطاقة ائتمان.' : 'Join 45,000+ students already learning. Start with a free masterclass — no credit card required.' ) ); ?>
      </p>
      <div style="display:flex;gap:var(--space-md);justify-content:center;flex-wrap:wrap;">
        ...
        <a href="<?php echo esc_url( $catalog_link ); ?>" class="btn btn-lg btn-outline" style="border-color:rgba(255,255,255,0.6);color:white;"><?php wpm_is_rtl() ? _e('تصفح جميع الدورات', 'edtech') : _e('Browse All Courses', 'edtech'); ?></a>
      </div>
    </div>
  </div>
</section>
