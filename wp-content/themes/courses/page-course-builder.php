  ...
  <div class="container container-md">
    <?php if ( ! is_user_logged_in() ) : ?>
      <div class="card" style="text-align:center;padding:var(--space-xl);">
        <p style="color:var(--color-text-muted);margin-bottom:var(--space-md);"><?php is_rtl() ? _e('سجّل الدخول لإنشاء دورة.', 'edtech') : _e('Sign in to create a course.', 'edtech'); ?></p>
    ...
    <div class="card reveal">
      ...
      <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
        ...
        <div class="form-group">
          ...
          <input id="builder-title" name="course_title" type="text" class="input-field" placeholder="<?php is_rtl() ? _e('تطوير الويب المتكامل', 'edtech') : _e('Full-Stack Web Development', 'edtech'); ?>" required>
        </div>
        <div class="form-group">
          <label class="form-label" for="builder-desc"><?php is_rtl() ? _e('وصف الدورة', 'edtech') : _e('Course Description', 'edtech'); ?></label>
          <textarea id="builder-desc" name="course_description" class="input-field" rows="4" placeholder="<?php is_rtl() ? _e('صف محتوى الدورة وما سيتعلمه الطالب...', 'edtech') : _e('Describe the course content and what students will learn...', 'edtech'); ?>"></textarea>
        </div>
        <div class="form-group">
          <label class="form-label" for="builder-price"><?php is_rtl() ? _e('السعر ($)', 'edtech') : _e('Price ($)', 'edtech'); ?></label>
          <input id="builder-price" name="course_price" type="number" class="input-field" value="49" min="0" step="1">
        </div>
        <button type="submit" class="btn btn-primary"><?php is_rtl() ? _e('حفظ والمتابعة للخطوة التالية →', 'edtech') : _e('Save & Continue to Next Step →', 'edtech'); ?></button>
      </form>
    </div>
