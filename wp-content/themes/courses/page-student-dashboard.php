    ...
    <?php if ( ! $is_logged_in ) : ?>
      ...
      <div class="card reveal" style="padding:var(--space-xl);box-shadow:var(--shadow-xl);border-radius:var(--radius-lg);">
        ...
        <div id="auth-panel-login" style="display:<?php echo $active_tab === 'login' ? 'block' : 'none'; ?>;">
          <form method="post" action="<?php echo esc_url( home_url( '/student-dashboard' ) ); ?>">
    ...
    <?php else : ?>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:var(--space-md);margin-bottom:var(--space-lg);" class="reveal">
        <div>
          ...
          <p style="color:var(--color-text-muted);margin:0;"><?php is_rtl() ? _e( 'أنت مستمر في الدراسة منذ 🔥 5 أيام متتالية!', 'edtech' ) : _e( "You're on a 🔥 5 Day Study Streak!", 'edtech' ); ?></p>
        </div>
        <div style="display:flex;gap:var(--space-sm);align-items:center;">
          ...
          <a href="<?php echo esc_url( wp_logout_url( home_url( '/student-dashboard' ) ) ); ?>" class="btn btn-ghost" style="color:var(--color-accent);"><?php is_rtl() ? _e( 'تسجيل الخروج', 'edtech' ) : _e( 'Log Out', 'edtech' ); ?></a>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 300px;gap:var(--space-xl);">
        <div>
          ...
          <div class="card reveal" style="background:linear-gradient(135deg,var(--color-primary) 0%,var(--color-secondary) 100%);color:white;padding:var(--space-xl);margin-bottom:var(--space-lg);">
            ...
            <h2 style="color:white;margin-bottom:6px;"><?php is_rtl() ? _e( 'تطوير الويب المتكامل / Full-Stack Development', 'edtech' ) : _e( 'Full-Stack Web Development', 'edtech' ); ?></h2>
            <p style="opacity:0.85;font-size:14px;"><?php is_rtl() ? _e( 'الدرس 1.3: تعمق في JavaScript ES2025 و Node.js', 'edtech' ) : _e( 'Lesson 1.3: JavaScript ES2025 & Node.js Deep Dive', 'edtech' ); ?></p>
          ...
          <div class="card reveal">
            ...
            <div style="display:grid;gap:var(--space-md);">
              <?php
              $my_courses = get_posts( array( 'post_type' => 'course', 'posts_per_page' => 3 ) );
              if ( ! empty( $my_courses ) ) :
                foreach ( $my_courses as $mc ) :
                  $mc_thumb = get_the_post_thumbnail_url( $mc->ID, 'medium' ) ?: 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=300&auto=format&fit=crop&q=80';
                  ?>
                  <div style="display:flex;gap:var(--space-md);align-items:center;padding:var(--space-sm);background:var(--color-bg-subtle);border-radius:var(--radius-md);">
                    ...
                    <div style="flex-grow:1;">
                      <h4 style="font-size:14px;margin-bottom:2px;"><?php echo esc_html( get_the_title( $mc->ID ) ); ?></h4>
                    ...
                    <a href="<?php echo esc_url( home_url( '/lesson-workspace' ) ); ?>" class="btn btn-secondary" style="font-size:12px;padding:6px 12px;"><?php is_rtl() ? _e( 'دخول', 'edtech' ) : _e( 'Open', 'edtech' ); ?></a>
                  </div>
                  <?php
                endforeach;
              endif;
        ...
        <aside>
          <div class="card reveal" style="margin-bottom:var(--space-lg);">
            ...
            <p style="font-size:14px;color:var(--color-text-muted);margin-bottom:var(--space-sm);"><?php is_rtl() ? _e( 'ساعات المشاهدة: 4.3س من 12.5س (35%)', 'edtech' ) : _e( 'Hours Watched: 4.3h of 12.5h (35%)', 'edtech' ); ?></p>
            <a href="<?php echo esc_url( home_url( '/certificates' ) ); ?>" class="btn btn-secondary" style="width:100%;"><?php is_rtl() ? _e( 'عرض الشهادات', 'edtech' ) : _e( 'View Certificates', 'edtech' ); ?></a>
          </div>
        </aside>
      </div>
